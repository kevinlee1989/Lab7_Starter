describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for(let i = 0; i < prodItemsData.length; i++){
      ...
      if (!product.title || product.price.length === 0) { allArePopulated = false; }
      if (!product.title || product.image.length === 0) { allArePopulated = false; }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // After adding the product check if the inner context become "Remove from Cart"
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (let i = 0; i < productItems.length; i++) {
      ...
      // Checking the current button 
      const innerTextHandle = await button.getProperty('innerText');
      const innerText = await innerTextHandle.jsonValue();

      // click if the inner text is 'Remove from Cart'
      if (innerText === 'Remove from Cart') {
        await button.click();
      }
    }

    // Checking the final count if it is 0
    const cartCount = await page.$eval('#cart-count', el => el.innerText);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const productItems = await page.$$('product-item');
    // set all the button as true
    let allButtonsCorrect = true;

    // Check if the all button's text are 'Add to Cart'
    for (let i = 0; i < productItems.length; i++) {
      const shadowRoot = await productItems[i].getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      const innerTextHandle = await button.getProperty('innerText');
      ...
      // if not 'Add to Cart' then set the boolean value to false
      if (innerText !== 'Add to Cart') {
        allButtonsCorrect = false;
        break;
      }
    }
    // Check the final count if it is 0
    const cartCount = await page.$eval('#cart-count', el => el.innerText);
